...
module.exports = {
  data: {
    name: "Check Member Permission In Channel",
  },
  ...
  subtitle: (data, constants) => {
    return `Member: ${constants.user(data.member)} - Channel: ${constants.channel(data.channel)} - Check: ${data.permission}`
  },
  ...
  async run(values, message, client, bridge) {
    ...
    let memberPermissionsInChannel = channel.permissionsOf(member.member)

    if (memberPermissionsInChannel.has(permission)) {
